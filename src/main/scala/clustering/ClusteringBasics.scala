      ...
      ports.foreach { port =>
        val config = ConfigFactory.parseString(
          s"""
             ...
             |""".stripMargin)
          .withFallback(ConfigFactory.load("clustering/clusteringBasic.conf"))
        val system = ActorSystem("RTJVMCluster", config) // all the actor system in a cluster has the same name
...
object ClusteringBasics_ManualRegistration extends App{
  val system = ActorSystem("RTJVMCluster", ConfigFactory.load("clustering/clusteringBasic.conf").getConfig("manualRegistration"))
  ...
  def joinExistingNode =
    cluster.join(Address("akka","RTJVMCluster","localhost",53847))
